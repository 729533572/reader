...
/**
 * Created by luhe on 17/2/21.
 */

public class MFSAutoScrollTextView extends TextView implements View.OnClickListener{
    ...
    public MFSAutoScrollTextView(Context context) {
        this(context, null);
    }

    public MFSAutoScrollTextView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MFSAutoScrollTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.MFSAutoScrollTextView);
        try {
            switchDuaration = array.getInt(R.styleable.MFSAutoScrollTextView_switchDuaration, DEFAULT_SWITCH_DURATION);
            idleDuaration = array.getInt(R.styleable.MFSAutoScrollTextView_idleDuaration, DEFAULT_IDLE_DURATION);
            switchOrientation = array.getInt(R.styleable.MFSAutoScrollTextView_switchOrientation, 0);
        } finally {
            array.recycle();
        }
        init();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.d("viclee", "onMeasure width is " + mWidth);
//
...
//        Log.d("viclee", "verticalOffset is " + verticalOffset);
        ...
        if (switchOrientation == 0) {//向上滚动切换
            if (verticalOffset > 0) {
                ...
                Rect bounds = new Rect();
                mPaint.getTextBounds(outStr, 0, outStr.length(), bounds);
                Integer textWidth = bounds.width();
                paddingLeft = (mWidth-textWidth)/2;

                canvas.drawText(outStr, paddingLeft, verticalOffset, mPaint);
            } else {
                Rect bounds = new Rect();
                mPaint.getTextBounds(inStr, 0, inStr.length(), bounds);
                Integer textWidth = bounds.width();
                paddingLeft = (mWidth-textWidth)/2;
