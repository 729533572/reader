 ...
 * 
 */
public class MFSGestureLockViewGroup extends RelativeLayout
{
	 ...
	 */
	private MFSGestureLockView[] mGestureLockViews;
	/**
	...
	public MFSGestureLockViewGroup(Context context, AttributeSet attrs)
	{
			...
			int defStyle)
	{
		...
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
				R.styleable.MFSGestureLockViewGroup, defStyle, 0);
		int n = a.getIndexCount();

		for (int i = 0; i < n; i++)
		{
			int attr = a.getIndex(i);
			if (attr == R.styleable.MFSGestureLockViewGroup_color_no_finger_inner_circle) {
				mNoFingerInnerCircleColor = a.getColor(attr,
						mNoFingerInnerCircleColor);

			} else if (attr == R.styleable.MFSGestureLockViewGroup_color_no_finger_outer_circle) {
				mNoFingerOuterCircleColor = a.getColor(attr,
						mNoFingerOuterCircleColor);

			} else if (attr == R.styleable.MFSGestureLockViewGroup_color_finger_on) {
				mFingerOnColor = a.getColor(attr, mFingerOnColor);

			} else if (attr == R.styleable.MFSGestureLockViewGroup_color_finger_up) {
				mFingerUpColor = a.getColor(attr, mFingerUpColor);

			} else if (attr == R.styleable.MFSGestureLockViewGroup_count) {
				mCount = a.getInt(attr, 3);

			} else if (attr == R.styleable.MFSGestureLockViewGroup_tryTimes) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		{
			mGestureLockViews = new MFSGestureLockView[mCount * mCount];
			...
			for (int i = 0; i < mGestureLockViews.length; i++)
			{
				...
				mGestureLockViews[i] = new MFSGestureLockView(getContext(),
				...
				lockerParams.setMargins(leftMagin, topMargin, rightMargin,
						bottomMargin);
				mGestureLockViews[i].setMode(MFSGestureLockView.Mode.STATUS_NO_FINGER);
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_MOVE:
			...
			mPaint.setAlpha(50);
			MFSGestureLockView child = getChildIdByPos(x, y);
			if (child != null)
			{
				int cId = child.getId();
				...
				{
					mChoose.add(cId);
					child.setMode(MFSGestureLockView.Mode.STATUS_FINGER_ON);
		...
		case MotionEvent.ACTION_UP:
			...
			for (int i = 0; i + 1 < mChoose.size(); i++)
			{
				...
				MFSGestureLockView startChild = (MFSGestureLockView) findViewById(childId);
				MFSGestureLockView nextChild = (MFSGestureLockView) findViewById(nextChildId);
	...
	{
		for (MFSGestureLockView gestureLockView : mGestureLockViews)
		{
			if (mChoose.contains(gestureLockView.getId()))
			{
				gestureLockView.setMode(MFSGestureLockView.Mode.STATUS_FINGER_UP);
			}
		}
	}
	...
	public void reset()
	{
		...
		for (MFSGestureLockView gestureLockView : mGestureLockViews)
		{
			gestureLockView.setMode(MFSGestureLockView.Mode.STATUS_NO_FINGER);
	 ...
	 * @return
	 */
	private MFSGestureLockView getChildIdByPos(int x, int y)
	{
		for (MFSGestureLockView gestureLockView : mGestureLockViews)
		{
