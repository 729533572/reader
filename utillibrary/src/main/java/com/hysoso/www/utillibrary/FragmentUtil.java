...
public class FragmentUtil
{
	...
	public void chooseFragments(int position,List<Fragment> fragments,Integer containViewId) {
		for (int i=0;i<fragments.size();i++){
			if (i==position){
				if (fragments.get(position).isAdded()){
					mManager.beginTransaction().show(fragments.get(position)).commit();
				}else {
					mManager.beginTransaction().add(containViewId, fragments.get(position)).commit();
				}
			}else {
	...
	public FragmentUtil remove(Fragment... fragments)
	{
	...
	public FragmentUtil show(Fragment... fragments)
	{
		if (fragments != null && fragments.length > 0)
		{
			FragmentTransaction transaction = beginTransaction();
	...
	public FragmentUtil hide(Fragment... fragments)
	{
		if (fragments != null && fragments.length > 0)
		{
			FragmentTransaction transaction = beginTransaction();
