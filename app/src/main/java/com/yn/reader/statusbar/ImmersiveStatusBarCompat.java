...
public class ImmersiveStatusBarCompat {
    ...
    public static void setStatusBarColor(Activity activity, int color, boolean lightStatusBar) {
        setStatusBarColor(activity.getWindow(), false, color, lightStatusBar);
    }

    public static void setStatusBarColor(Activity activity, boolean fullscreen,
                                         int color, boolean lightStatusBar) {
        setStatusBarColor(activity.getWindow(), fullscreen, color, lightStatusBar);
    }

    public static void setStatusBarColor(Activity activity, boolean fullscreen, int color) {
        boolean isLightColor = toGrey(color) > 225;
        setStatusBarColor(activity.getWindow(), fullscreen, color, isLightColor);
    }
     ...
     * @param lightStatusBar if the status bar color is light. Only effective in some devices.
     */
    public static void setStatusBarColor(Window window, boolean fullscreen, int color, boolean lightStatusBar) {
        ...
        IMPL.setStatusBarColor(window, fullscreen, color);
    ...
    public static void setFitsSystemWindows(Window window, boolean fitSystemWindows) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            ...
            if (mChildView != null) {
                //注意不是设置 ContentView 的 FitsSystemWindows, 而是设置 ContentView 的第一个子 View . 预留出系统
                // View 的空间.
